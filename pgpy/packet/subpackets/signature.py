...
class IssuerFingerprint(Signature):
    ...
    def issuer_fingerprint(self) -> Fingerprint:
        if self._issuer_fpr is None:
            raise ValueError("tried to access the issuer fingerprint before it was set")
        return self._issuer_fpr

    @issuer_fingerprint.register
    def issuer_fingerprint_set(self, val:Union[str,bytearray,Fingerprint]) -> None:
