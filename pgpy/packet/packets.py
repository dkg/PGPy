...
class OnePassSignatureV6(OnePassSignature):
    ...
    def halg_set(self, val:int) -> None:
        ...
        if self._salt is not None and len(self._salt) != val.sig_salt_size:
            raise ValueError(f"v6 OPS: salt is already set to {len(self._salt)} octets, cannot set hash algorithm to {val!r} since it would require {val.sig_salt_size} octets.")
        self._halg = val

    def __bytearray__(self) -> bytearray:
        if (self.sigtype is None or
            self.halg is None or
            self.salt is None or
            self.pubalg is None):
        ...
        _bytes += self.salt
        _bytes += self.signer.__wireformat__()
    ...
    def parse(self, packet:bytearray) -> None:
        ...
        fpr_version:int = packet[0]
        del packet[0]
        if fpr_version != 6:
            raise ValueError("v6 OPS with signer key version {fpr_version}, should be 6")
...
class SKESessionKeyV6(SKESessionKey):
    ...
    *  An authentication tag for the AEAD mode.
    ...
    @property
    def symalg(self) -> Optional[SymmetricKeyAlgorithm]:
        return self._cipher_algo

    def __init__(self) -> None:
        super().__init__()
        self._cipher_algo:SymmetricKeyAlgorithm = SymmetricKeyAlgorithm.AES128
        self._aead_algo:AEADMode = AEADMode.OCB
        self.s2k = S2KSpecifier()
    ...
    def __bytearray__(self) -> bytearray:
        ...
        s2k_field:bytearray = self.s2k.__bytearray__()

        _bytes.append(3 + len(s2k_field) + self._aead_algo.iv_len)
        _bytes.append(self._cipher_algo)
    ...
    def _get_info(self) -> bytes:
        ...
        return bytes([0b11000000 + self.__typeid__, self.__ver__, int(self._cipher_algo), int(self._aead_algo)])

    def _get_derived_key(self, passphrase:Union[str,bytes]) -> bytes:
        s2k_derived_key = self.s2k.derive_key(passphrase, self._cipher_algo.key_size)
        hkdf = HKDF(algorithm=SHA256(), length=self._cipher_algo.key_size // 8, salt=None, info=self._get_info())
    ...
    def _get_aead(self, passphrase:Union[str,bytes]) -> AEAD:
        ...
        return AEAD(self._cipher_algo, self._aead_algo, derived_key)
    ...
    def parse(self, packet:bytearray) -> None:
        ...
        self._cipher_algo = SymmetricKeyAlgorithm(packet[0])
        del packet[0]
...
class PubKeyV6(PubKeyV4):
    ...
    def __init__(self):
        super(PubKeyV6, self).__init__()

    def __bytearray__(self):
        _bytes = bytearray()
        # note: the line below uses super(PubKeyV4), not
        # super(PubKeyV6), because we want to avoid injecting a v4
        # version of the pubkey.
        _bytes += super(PubKeyV4, self).__bytearray__()
        _bytes += self.int_to_bytes(calendar.timegm(self.created.timetuple()), 4)
        _bytes += self.int_to_bytes(self.pkalg)
...
class PrivKeyV6(PrivKey, PubKeyV6):
    ...
    def pubkey(self):
        ...
        elif self.pkalg in {PubKeyAlgorithm.X25519, PubKeyAlgorithm.X448,
                            PubKeyAlgorithm.Ed25519, PubKeyAlgorithm.Ed448}:
    ...
    def protect(self, passphrase:str,
                ...
                s2kspec:Optional[S2KSpecifier]=None,
                iv:Optional[bytes]=None,
                aead_mode:Optional[AEADMode]=AEADMode.OCB) -> None:
        if enc_alg is None:
            enc_alg = SymmetricKeyAlgorithm.AES256
        self.keymaterial.encrypt_keyblob(passphrase, aead_mode=aead_mode, s2kspec=s2kspec, enc_alg=enc_alg, iv=iv,
                                         packet_tag=self.__typeid__,
                                         key_version=self.__ver__,
                                         creation_time=self._created)
        del passphrase
...
class IntegrityProtectedSKEDataV2(IntegrityProtectedSKEData):
    ...
    def decrypt(self, key:bytes, algo:Optional[SymmetricKeyAlgorithm]=None) -> bytearray:
        if algo is not None:
            raise PGPDecryptionError(f"v2 SEIPD knows its own algorithm ({self.cipher!r}), should not be explicitly passed one, but it got {algo!r} (maybe v3 PKESK or v4 SKESK precedes it instead of v6?)")
        aead,iv = self._get_aead_and_iv(key)
        ad = self._get_info()
