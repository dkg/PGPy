...
class S2KSpecifier(Field):
    '''
    Just the S2K specifier and its various options

    FIXME: use polymorphism here instead?
    ...
    def __init__(self,
                 ...
                 smartcard_serial:Optional[ByteString] = None,
                 argon2_time:int = 1,
        ...
        if s2ktype == String2KeyType.Argon2:
            ...
            if (1 << argon2_memory_exp) < 8*argon2_parallelism:
                raise ValueError(f"Argon2 memory size in KiB (m={argon2_memory_exp}, or {1 << argon2_memory_exp}KiB) should be at least parallelism({argon2_parallelism})*8")
        super().__init__()
        self._specifier:String2KeyType = s2ktype
        self._halg:HashAlgorithm = halg
        self._salt:Optional[bytes] = None
        if salt is not None:
            self._salt = bytes(salt)
        self._count = 65011712 # the default!
        if s2ktype == String2KeyType.Iterated:
    ...
    def __copy__(self) -> "S2KSpecifier":
        s2k = S2KSpecifier()
        s2k._specifier = self._specifier
    ...
    def iteration_count(self) -> int:
        ...
        if self._specifier != String2KeyType.Iterated:
            raise ValueError(f"Cannot retrieve iteration count on S2KSpecifier Type {self._specifier!r}")
    ...
    def iteration_count_int(self, val:int) -> None:
        if self._specifier is None:
            self._specifier = String2KeyType.Iterated
        if self._specifier != String2KeyType.Iterated:
            raise ValueError(f"Cannot set iteration count on S2KSpecifier type {self._specifier!r}")
    ...
    def iteration_octet(self) -> Optional[bytes]:
        if self._specifier != String2KeyType.Iterated or self._count is None:
            return None
    ...
    def salt(self) -> bytes:
        ...
        if self._salt is None:
            self._salt = os.urandom(self._specifier.salt_length)
        return self._salt
    @salt.register
    ...
    def smartcard_serial(self) -> Optional[bytes]:
        if self._specifier != String2KeyType.GNUExtension or self._gnupg_extension != S2KGNUExtension.Smartcard:
            return None
        return self._smartcard_serial
    @smartcard_serial.register
    def smartcard_serial_bytes(self, val:Union[bytes,bytearray]) -> None:
        if self._specifier != String2KeyType.GNUExtension:
            raise ValueError(f"smartcard serial number can only be set for String2KeyType GNUExtension, not {self._specifier!r}")
    ...
    def __bytearray__(self) -> bytearray:
        _bytes = bytearray()
        _bytes.append(self._specifier)
        if self._specifier == String2KeyType.GNUExtension:
            return self._gnu_bytearray(_bytes)
        if self._specifier in [String2KeyType.Simple, String2KeyType.Salted, String2KeyType.Iterated]:
            _bytes.append(self._halg)
        _bytes += self.salt
        if self._specifier == String2KeyType.Iterated:
            _bytes += self.iteration_octet
        if self._specifier == String2KeyType.Argon2:
    ...
    def parse(self, packet) -> None:
        self._specifier = String2KeyType(packet[0])
        del packet[0]
        ...
        if self._specifier.salt_length > 0:
            self._salt = bytes(packet[:self._specifier.salt_length])
            del packet[:self._specifier.salt_length]
    ...
    def _gnu_bytearray(self, _bytes):
        if self._specifier != String2KeyType.GNUExtension:
            raise ValueError(f"This is not a GnuPG-extended S2K specifier ({self._specifier})")
    ...
    def derive_key(self, passphrase:Union[str,bytes], keylen_bits:int) -> bytes:
        if self._specifier not in [String2KeyType.Simple, String2KeyType.Salted, String2KeyType.Iterated, String2KeyType.Argon2]:
            raise NotImplementedError(f"Cannot derive key from S2KSpecifier {self._specifier}")
        ...
        count = base_count
        if self._specifier == String2KeyType.Iterated and self._count > count:
            count = self._count
...
class String2Key(Field):
    ...
    """
    @sdproperty
    def encalg(self) -> Optional[SymmetricKeyAlgorithm]:
        return self._encalg

    @encalg.register
    def encalg_int(self, val:Union[int,SymmetricKeyAlgorithm]):
    ...
    @specifier.register
    def specifier_int(self, val:Union[int,String2KeyType]):
        self._specifier._specifier = String2KeyType(val)

    @sdproperty
    def salt(self) -> bytes:
        return self._specifier.salt
    @salt.register
    def salt_bytes(self, val:Union[bytes,bytearray]) -> None:
        self._specifier.salt = val

    @sdproperty
    def gnuext(self) -> Optional[S2KGNUExtension]:
        return self._specifier._gnupg_extension

    @gnuext.register
    def gnuext_int(self, val:Union[int,S2KGNUExtension]):
        self._specifier._gnupg_extension = S2KGNUExtension(val)

    @sdproperty
    def scserial(self) -> Optional[bytes]:
        return self._specifier.smartcard_serial
    @scserial.register
    def scserial_bytes(self, val:Union[bytes,bytearray]) -> None:
        self._specifier.smartcard_serial = val

    @sdproperty
    def halg(self) -> Optional[HashAlgorithm]:
        return self._specifier._halg
    ...
    def __copy__(self) -> "String2Key":
        ...
        if self._encalg is not None:
            s2k._encalg = self._encalg
